...
def get_market_data():
    """Получение метрик деривативов напрямую через бесплатное API биржи"""
    try:
...
def generate_content():
    """Генерация полноценного контента (80% сетапы / 20% статьи) с интеграцией реф-ссылки"""
    date_str = datetime.now().strftime("%Y-%m-%d %H:%M")
    data = get_market_data()
    
    # Текст для реферального блока на трех языках
    ref_ru = f"<br><br><strong>Где реализовать этот сетап?</strong> Используйте профессиональные AI-инструменты, минимальные комиссии и торгуйте с гарантированной безопасностью резервов на бирже Bitget. <a href='{REF_LINK}' target='_blank'>Зарегистрируйтесь сейчас и заберите Welcome Pack до 6,200 USDT</a>."
    ref_en = f"<br><br><strong>Where to trade this setup?</strong> Capitalize on lowest fees, advanced AI-automation, and 100% proven proof of reserves on Bitget. <a href='{REF_LINK}' target='_blank'>Register via our link to claim your Welcome Pack up to 6,200 USDT</a>."
    ref_es = f"<br><br><strong>¿Dónde operar este setup?</strong> Aproveche las comisiones más bajas, herramientas de IA avanzadas y una Prueba de Reservas del 100% en Bitget. <a href='{REF_LINK}' target='_blank'>Regístrese aquí y reclame su Welcome Pack de hasta 6,200 USDT</a>."

    if random.randint(1, 100) <= 80:
        # --- ТОРГОВЫЙ СЕТАП (80% контента) ---
        ...
        topics = [
            ...
            },
            {
                ...
                "es": {"badge": "Seguridad", "title": "La seguridad del dispositivo: el pilar invisible de sus fondos", "desc": f"Muchos analizan patrones gráficos durante horas pero olvidan la seguridad digital básica. Si el sistema operativo de su teléfono está comprometido, la protección de sus contraseñas es inexistente. Donde termina la privacidad, los fondos desaparecen rápidamente. Opere con confianza utilizando infraestructura de nivel global.{ref_es}"}
            }
        ]
        chosen = random.choice(topics)
        return {
            "date": date_str,
            "ru": {"badge": chosen["ru"]["badge"], "title": chosen["ru"]["title"], "description": chosen["ru"]["desc"]},
            "en": {"badge": chosen["en"]["badge"], "title": chosen["en"]["title"], "description": chosen["en"]["desc"]},
            "es": {"badge": chosen["es"]["badge"], "title": chosen["es"]["title"], "description": chosen["es"]["desc"]}
        }

def upload_to_github(post_data):
    filename = f"articles/post_{int(datetime.now().timestamp())}.json"
    url = f"https://api.github.com/repos/{REPO}/contents/{filename}"
    headers = {"Authorization": f"token {GH_TOKEN}", "Accept": "application/vnd.github.v3+json"}
    
    import base64
    content = json.dumps(post_data, ensure_ascii=False, indent=4)
    b64_content = base64.b64encode(content.encode('utf-8')).decode('utf-8')
    
    res = requests.put(url, headers=headers, json={"message": "Automated professional content publishing", "content": b64_content})
    print("Success" if res.status_code in [200, 201] else f"Error: {res.text}")

if __name__ == "__main__":
    post = generate_content()
    upload_to_github(post)
